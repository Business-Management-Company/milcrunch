    ...
    <div className="min-h-screen bg-[#0A0F1E] p-6 md:p-10">
      ...
      <div className="grid lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 space-y-4">
          {TOOLS.map((tool, i) => (
            <div key={tool.name} className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-5">
              <div className="flex justify-between items-center mb-3">
                <div>
                  <span className="text-sm font-medium text-gray-300">{tool.category}</span>
                </div>
                <span className="text-sm font-bold text-white tabular-nums">{fmt(costs[i])}/mo</span>
              </div>
              <Slider
                value={[costs[i]]}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-6">
            <p className="text-xs font-semibold tracking-widest uppercase text-gray-500 mb-4">Your Current Stack</p>
            <div className="space-y-2">
              {TOOLS.map((tool, i) => (
                <div key={tool.name} className="flex justify-between text-sm">
                  <span className="text-gray-400">{tool.name}</span>
                  <span className="text-white tabular-nums">{fmt(costs[i])}</span>
                </div>
              ))}
              <div className="pt-3 mt-2 border-t border-white/[0.08] flex justify-between">
                <span className="text-sm font-bold text-white">Total</span>
                <span className="text-lg font-extrabold text-red-400 tabular-nums">{fmt(total)}/mo</span>
              </div>
            </div>
          </div>
          ...
          <div className={cn(
            ...
            <p className="text-xs font-semibold tracking-widest uppercase text-gray-500 mb-2">Monthly Savings</p>
            ...
            <p className={cn("text-sm mt-1", savings > 0 ? "text-[#10B981]/70" : "text-gray-500")}>
