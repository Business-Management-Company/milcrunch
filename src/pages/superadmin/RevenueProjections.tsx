...
export default function RevenueProjections() {
  ...
  const exportPdf = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>RecurrentX Revenue Projections — ${year.label}</title><style>body{font-family:system-ui;background:#0A0F1E;color:#fff;padding:40px}table{width:100%;border-collapse:collapse;margin-top:20px}h1{font-size:28px}h2{color:#6C5CE7;margin-top:8px}.total{font-size:24px;color:#10B981;font-weight:800;margin-top:24px}</style></head><body><h1>RecurrentX Revenue Projections</h1><h2>${year.label}</h2><table>${rows}<tr style="background:rgba(108,92,231,0.15)"><td style="padding:8px 12px;font-weight:700">Total Revenue</td><td style="padding:8px 12px;text-align:right;font-weight:700;color:#10B981;font-family:monospace">${fmt(total)}</td></tr></table><p class="total">Total: ${fmt(total)}</p><p style="color:#6B7280;font-size:12px;margin-top:40px">&copy; ${new Date().getFullYear()} RecurrentX &middot; Confidential</p></body></html>`);
    w.document.close();
